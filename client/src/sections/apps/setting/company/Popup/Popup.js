...
const getInitialValues = (customer) => {
  ...
  if (customer) {
    const newCustomer = {
      ...
      name: customer?.name,
      code: customer?.code,
      departmentSelect: customer?.dp_id,
      userSelect: customer?.department
    };
...
const AddCustomer = ({ customer, open, onCancel, namePage, status, fetchDepartment }) => {
  ...
  const handlebeforeClose = () => {
    if (isCreating) {
      ...
      } else if (namePage === 'position') {
        if (!formik.values.name || !formik.values.code || !formik.values.departmentSelect) {
          Swal.fire({
            title: 'Discard',
            text: `if you discard the change, this process will not be able to be undone.`,
            showCancelButton: true,
          ...
          formik.values.code !== customer.code ||
          formik.values.departmentSelect !== customer.dp_id
        ) {
          Swal.fire({
            title: 'Discard',
            text: `if you discard the change, this process will not be able to be undone.`,
  ...
  async function setModule() {
    if (namePage === 'department') {
      ...
      const workData = {
        name: formik.values.name,
        code: formik.values.code,
        department_Id: formik.values.departmentSelect
      };

      const createWorkData = await api.created.createPosition(workData);
      fetchDepartment()
      onCancel();
    } else if (namePage === 'group') {
      const workData = {
  ...
  async function updateModule(values) {
    if (namePage === 'department') {
      ...
      const updatedProject = {
        id: customer.id,
        code: formik.values.code,
        department_id: formik.values.departmentSelect || customer.dp_id,
        name: formik.values.name
      };
      const updateData = await api.company.updatePosition(updatedProject);
      fetchDepartment()
      onCancel();
    } else if (namePage === 'group') {
      const updatedProject = {
        id: customer.id,
        code: formik.values.code,
        user_id: formik.values.userSelect || customer.department,
        name: formik.values.name
      };
            ...
            {namePage === 'position' ? (
              <Grid item xs={12}>
                <InputLabel htmlFor="user">
                  {' '}
                  {namePage === 'department' ? 'Position' : namePage === 'position' ? 'Departmnet' : null} Select
                ...
                <Autocomplete
                  ...
                  value={dataDepart.find((option) => option.department_id === formik.values.departmentSelect) || ''}
                  onChange={(event, newValue) => {
                    if (newValue !== null) {
                      console.log('jaaaaa', newValue);
                      formik.setFieldValue('departmentSelect', newValue.department_id);
                      console.log('Selected Client ID:', newValue.department_id);
                    } else {
                      console.log('nullllllllll');
                      formik.setFieldValue('departmentSelect', '');
                    }
                  }}
                  fullWidth
                  margin="normal"
                  renderInput={(params) => (
                    <div style={{ position: 'relative' }}>
                      <TextField
                        ...
                        error={Boolean(touched.departmentSelect && errors.departmentSelect)}
                        helperText={touched.departmentSelect && errors.departmentSelect}
                        InputProps={{
                          ...params.InputProps,
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        placeholder={params ? '' : 'Select Member'}
                      />
                    )}
                  />
                </Grid>
