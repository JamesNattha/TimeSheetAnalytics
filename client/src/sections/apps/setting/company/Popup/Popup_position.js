...
const getInitialValues = (customer) => {
  ...
  if (customer) {
    ...
    const newCustomer = {
      id: customer?.position_id,
      name: customer?.position_name,
      code: customer?.position_code,
      departmentSelect: customer?.department_id
    };
...
const AddCustomer = ({ customer, open, onCancel, namePage, status, fetchDepartment }) => {
  ...
  async function updateModule(values) {
    const updatedProject = {
      id: customer.position_id,
      code: formik.values.code,
      department_id: formik.values.departmentSelect || customer.department_id,
      name: formik.values.name
    };
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <InputLabel htmlFor="user">
                {' '}
                {namePage === 'department' ? 'Position' : namePage === 'position' ? 'Department' : null} Select
