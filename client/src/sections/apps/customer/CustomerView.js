...
const WorkonhandView = ({ data }) => {
  ...
  const modifiedRole = useMemo(() => {
    if (data && role) {
      const filteredRoles = role.filter((person) => person.userId === data.createdBy);
  ...
  const modifiedUser = useMemo(() => {
    if (data && role) {
      const filteredRoles = role.filter((person) => person.userId === data.createdBy);
      if (filteredRoles.length > 0) {
        const user = filteredRoles[0];
        if (user && user.userId && user.username && user.firstName && user.lastName) {
          return {
            userId: user.userId,
  ...
  console.log('modifiedRole นี่แหะล ', modifiedUser);
                    ...
                    <Stack spacing={0.5} alignItems="center">
                      ...
                      {modifiedRole.map((role, index) => (
                        <Chip
                          ...
                          label={role}
                          size="small"
                          color={role === "admin" ? "warning" : role === "manager" ? "info" : role === "employee" ? "success" : "primary"}
                          sx={{
                            position: 'relative',
                            top: '5px',
                            p: 0.5,
                ...
                <Grid item xs={12}>
                  <List component="nav" aria-label="main mailbox folders" sx={{ py: 0 }}>
                    <ListItem>
                      ...
                      <ListItemSecondaryAction>
                        <Typography align="right">{modifiedUser.username}</Typography>
                      </ListItemSecondaryAction>
                    </ListItem>
                    <ListItem>
                      <ListItemIcon>
                      ...
                      <ListItemSecondaryAction>
                        <Typography align="right">
                          <NumberFormat displayType="text" format="+1 (###) ###-####" mask="_" defaultValue={modifiedUser.tel} />
                        </Typography>
                      </ListItemSecondaryAction>
                    </ListItem>
                    <ListItem>
                      ...
                      <ListItemSecondaryAction>
                        <Typography align="right">Address : {modifiedUser.address}</Typography>
                      </ListItemSecondaryAction>
                    </ListItem>
                  </List>
                </Grid>
            ...
            <Stack spacing={2.5}>
              <MainCard title="Work Details">
                <List sx={{ py: 0 }}>
                  ...
                  <ListItem divider={!matchDownMD}></ListItem>
                  <ListItem>
                    <Stack spacing={0.5}>
                      <Typography color="secondary">Note</Typography>
                      <Typography>{modifiedData.note}</Typography>
                    </Stack>
                  </ListItem>
                </List>
              </MainCard>
