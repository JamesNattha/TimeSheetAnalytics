...
import useNewPerson from '../Data/data_main_group';
import useColumns from '../Column/column_main_group';
import Swal from 'sweetalert2';
import SubItems from '../Sub/sub_group';
import AddPopup from 'sections/apps/setting/company/Popup/Popup_group';
...
const PositionList = () => {
  ...
  const fetchDepartment = async () => {
    try {
      const { data, isStatus } = await api.company.getGroup();
  ...
  const handleDeleteCustomer = (values) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetchDepartment();
        const deleted = api.company.deleteGroup(values).then(() => {
          fetchData();
          Swal.fire({
            title: 'Success',
            timer: 2000,
