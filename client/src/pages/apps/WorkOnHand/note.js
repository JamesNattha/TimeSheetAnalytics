          ...
          if (personColumn && personColumn.value && personColumn.value !== '') {
            ...
            if (ids.includes(selectUser)) {
              ...
              const row = {
                ...
                startDate: moment(board.startDate).format('DD-MM-YYYY'),
                endDate: moment(board.endDate).format('DD-MM-YYYY'),
                status: status ? <span key={status.id}>{status.text}</span> : null,
                email: Email,
                owner: Owners
              };
