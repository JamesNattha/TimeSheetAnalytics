...
const useNewPerson = (projectData, fetchData, timesheetData) => {
  const data = projectData;
  const datais = fetchData;
  const datats = timesheetData;
  console.log('raw data', data);
  console.log('datais in group', datais);
  // console.log('datats', datats.total);
  const formattedData = useMemo(() => {
    const rows = [];

    const calculateSumary = (int) => {
    ...
    if (data) {
      data.forEach((person, index) => {
        ...
        if (Array.isArray(datais)) {
          datais.forEach((group, index) => {
            console.log('group', group.User.username);
            const rowss = {
              position_code: group.User.username,
              position_name: `${group.User.first_name} ${group.User.last_name}`,
              profile: group.User.user_id
            };
          ...
          const row = {
            ...
            sum:
              calculateArraySumary(person.Works.map((item) => item.TimeSheetDTs.map((item) => item.total))) !== 0
                ? calculateSumary(person.Works.map((item) => item.total)) -
