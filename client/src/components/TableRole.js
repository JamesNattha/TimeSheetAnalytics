...
function ReactTable({ columns, data, getHeaderProps, renderRowSubComponent, handleAdd, nameCreateButton, sortColumn }) {
  const theme = useTheme();
  const filterTypes = useMemo(() => renderFilterTypes, []);

  const sortBy = useMemo(() => {
    return [{ id: sortColumn, desc: false }];
  }, [sortColumn]);
        ...
        <Stack spacing={2}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', margin: '10px', padding: '5px 7px 5px 7px', alignItems: 'center', borderRadius: '40px' }}>
                <FormControl sx={{ minWidth: 120 }}>
                  <Typography variant="subtitle1" component="span">
                    {/* {console.log('test', data)} */}
                    {/* {data.filter((item) => item.person.invite === true).length} Members available in your company. */} 
                    {data.length} Members available in your company.
                  </Typography>
                </FormControl>
              </div>
                  ...
                  <TableRow key={i} {...headerGroup.getHeaderGroupProps()} sx={{ '& > th:first-of-type': { width: '58px' } }}>
                    {headerGroup.headers.map((column, index) => (
                      <TableCell
                        ...
                        }} // Set the width for the TableCell
                      >
